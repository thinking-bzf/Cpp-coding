...
void ShowMenu();
void Intersections(LinkList<int> &, LinkList<int> &);
void Subtraction(LinkList<int> &, LinkList<int> &);
void UnionofTwoLists(LinkList<int> &, LinkList<int> &);
void Merge(LinkList<int> &, LinkList<int> &);
void SubsetAndComplement(LinkList<int> &, LinkList<int> &);
void SumOfX(LinkList<int> &);
void OrderListInsert(LinkList<int> &);
void IntervalDelete(LinkList<int> &list1);
void JosephusProblem(LinkList<int> &);
int main()
{
    int choice;
    static uniform_int_distribution<int> nums(5, 150);
    LinkList<int> list1(50);
    LinkList<int> list2(50);
    LinkList<int> list3;
    for (int i = 0; i < 50; i++)
...
void Intersections(LinkList<int> &list1, LinkList<int> &list2)
{
...
void Subtraction(LinkList<int> &list1, LinkList<int> &list2)
{
    DeleteDuplicate(list1);
    DeleteDuplicate(list2);
    cout << "The first sequence list is:" << endl;
    ...
    {
        list1.GetElem(value, i);
        int index = list2.LocateElem(value);
        if (index != -1)
            list3.Delete(value, list3.LocateElem(value));
    }
    cout << "The different set of sequence list is:" << endl;
    list3.Show();
}
void UnionofTwoLists(LinkList<int> &list1, LinkList<int> &list2)
{
    cout << "The first sequence list is:" << endl;
    list1.Show();
    cout << endl;
...
void SortList(LinkList<int> &list)
{
...
void Merge(LinkList<int> &list1, LinkList<int> &list2)
{
    SortList(list1);
    SortList(list2);
...
void ImportFromFile(LinkList<int> &list, string Path)
{
...
void SubsetAndComplement(LinkList<int> &list1, LinkList<int> &list2)
{
    // 由于随机数计算是否是子集有点困难，所以使用文件导入的自定义方式
    cout << "The first sequence list is:" << endl;
    // 分别导入两个列表并去重
    ImportFromFile(list1, "Subset1.txt");
    DeleteDuplicate(list1);
    list1.Show();
    cout << endl
         << "The second sequence list is:" << endl;
    ImportFromFile(list2, "Subset2.txt");
    DeleteDuplicate(list2);
    list2.Show();
    LinkList<int> list3(list1);
    int SubsetFlag = 1;
...
void OrderListInsert(LinkList<int> &list)
{
    int insertion;
    SortList(list);
...
void IntervalDelete(LinkList<int> &list)
{
    int low, high;
    SortList(list);
    ...
    input(&high);
    int lowIndex = 1;
    int highIndex = list.Length();
    int lowvalue, highvalue;
    list.GetElem(lowvalue, lowIndex);
    list.GetElem(highvalue, highIndex);
    while (lowvalue < low)
    {
        lowIndex++;
        list.GetElem(lowvalue, lowIndex);
...
void JosephusProblem(LinkList<int> &list)
{
