...
struct Node
{
    ...
    Node *lchild;     //左孩子结点指针
    Node *rchild;  //右孩子结点指针
    Node* parent; //父结点指针，使用优先队列后，这个指针没什么用      
};
...
public:
    ...
    string ScanCharacter();   //统计源文件中字符的种类及个数的函数
...
private:
	...
	//字符char 对应的 Huffman 编码 
	// 'a' -> "10110"
	map<char, string>  mapHCode; 
	...
	*/	
	...
	//按权重排过序的字符序列 
	vector< pair<char,int> > vecSortWeight;
...
//从键盘获取输入文本 
bool HuffmanTree::_GetTextFromKeyboard(const char* txt) 
{
    //依次读入每个字符，一个字节
	//中文占两个字节，所以分两次读入
	int i=0;
    ...
    while( (ch=txt[i++]) && (ch!='\0') ) 
    {
    	//ch是关键字，对map<char,int>中的int部分赋值；
		//默认的int初始值为0 
        mapCharWeight[ch]++;    
	}
...
bool HuffmanTree::_GetTextFromFile(const char* srcfile)
{
    ...
    while(infile.get(ch)) 
    {
		//ch是关键字，对map<char,int>中的int部分赋值；默认的int初始值为0 
        mapCharWeight[ch]++;  
    }
    infile.close();
...
void  showMenu()
{
	   ...
	   cout<<"\t$                                                            $"<<endl;
	   cout<<"\t$                                                            $"<<endl;
	   cout<<"\t$       5. 文件压缩(加密)        6. 解压文件(加密)           $"<<endl;
	   cout<<"\t$                                                            $"<<endl;
	   cout<<"\t$                                                            $"<<endl;
	   cout<<"\t$       7. 保存到文件            8. 退出                     $"<<endl;
	   cout<<"\t$                                                            $"<<endl;
	   cout<<"\t$                                                            $"<<endl;
...
void  showEnglishMenu()
{
	   ...
	   cout<<"\t$                                                            $"<<endl;
	   cout<<"\t$    3. Create BT(Pre-In Order) 4. Create BT(Post-In Order)  $"<<endl;
	   cout<<"\t$                                                            $"<<endl;
